// Dinosaur Division Game with Randomized Times Tables (2, 5, 10)
let cleanup = null;

const LEVELS = [
  {
    name: '2 Times Table Division',
    description: '2 ÷ 2 to 10',
    table: 2,
    quotients: [1, 2, 3, 4, 5],
    hasClues: true,
  },
  {
    name: '5 Times Table Division',
    description: '5 ÷ 5 to 25',
    table: 5,
    quotients: [1, 2, 3, 4, 5],
    hasClues: true,
  },
  {
    name: '10 Times Table Division',
    description: '10 ÷ 10 to 50',
    table: 10,
    quotients: [1, 2, 3, 4, 5],
    hasClues: true,
  },
  ...
  {
    name: 'Mixed Times Tables Division',
...
function generateQuestion(level, lastQuotient = null) {
  if (level.table !== null) {
    let quotient;
    do {
...
export function init(container, options = {}) {
  ...
  function nextQuestion(scoreCounter) {
    ...
    if (shuffledLevels[levelIdx].hasClues) {
      ...
      // Simple grouping: show dividend items divided into groups of table
      const groups = [];
    ...
    function checkAnswer() {
      ...
      // Check answer
      if (userAnswer === currentQuestion.quotient) {
        // Correct answer
        score++;
        questionCount++;
        ...
        if (levelIdx < 6 && questionCount === 8) {
          ...
          // Next question (for level 7, keep going)
          setTimeout(() => {
            submit.disabled = false;
            input.disabled = false;
            nextQuestion(scoreCounter);
  ...
  function showLevelUpCelebration(next) {
    ...
    celebMsg.textContent = 'Great job! Division Level Complete!';
  ...
  function endGame(scoreCounter, wrongQuestion = null) {
    ...
    if (wrongQuestion !== null) {
      ...
      correctMsg.textContent = `Correct answer: ${wrongQuestion.dividend} ÷ ${wrongQuestion.table} = ${wrongQuestion.quotient}`;
